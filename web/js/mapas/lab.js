	...
	create()
	{
		...
		npc.forEach(obj => {
			...
			if(obj.name == 'SkullAG')
			{
				...
				skull.create(obj, sc.config)// = this.add.sprite(obj.x, obj.y-8, 'SkullAG').setDepth(5)
				obj.destroy()
			}
			if(obj.name == 'bombilla')
			{
				lights.unshift(scene.lights.addLight(obj.x, obj.y, 1000, 0xffffff).setIntensity(2))
				obj.destroy()
			}
		})
